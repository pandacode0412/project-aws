...
import { useCreateExercise } from '../../hooks/queries/useExerciseQueries';
...
export const AddExerciseModal: React.FC<AddExerciseModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: 'Lỗi',
        description: (error as Error)?.message || 'Không thể tạo bài tập mới',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Thêm bài tập mới</ModalHeader>
        <ModalCloseButton />
          ...
          <VStack spacing={4}>
            ...
            {createExerciseMutation.error && (
              <Alert status="error" borderRadius="md">
                <AlertIcon />
                {createExerciseMutation.error.message || 'Có lỗi xảy ra khi tạo bài tập'}
              </Alert>
            )}
          </VStack>
        </ModalBody>
        ...
        <ModalFooter>
          <Button
            ...
            mr={3}
            onClick={onClose}
            isDisabled={createExerciseMutation.isPending}
          >
            Hủy
          </Button>
          <Button
